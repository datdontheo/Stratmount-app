...
const roleBadgeColors = {
  ADMIN: 'bg-white/10 text-text-primary',
  ...
  return (
    <header className="flex items-center justify-between px-4 lg:px-6 py-3 bg-bg-secondary border-b border-border">
      ...
      <div className="flex items-center gap-3">
        ...
        <span
          className={`text-xs font-medium px-2.5 py-1 rounded-full font-heading ${roleBadgeColors[user?.role] || 'bg-white/10 text-text-primary'}`}
        >
          {user?.role}
        </span>
